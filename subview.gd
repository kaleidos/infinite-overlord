...
func _ready():
	get_parent().get_parent().set_focus_mode(2)
	get_parent().get_parent().grab_focus()
	print(get_parent().get_parent().has_focus())
...
func _button_pressed(viewport, event, shape, node):
	...
	if (event.type == InputEvent.MOUSE_BUTTON && event.button_index == BUTTON_LEFT && event.pressed == 0):
		if currentNodeSelected != null:
			currentNodeSelected.remove_child(currentNodeSelected.get_node("selected"))
